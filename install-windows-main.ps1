...
#    8. GitHub CLI, Vercel CLI 확인/설치 (선택 도구 - 실패해도 계속 진행)
#    9. 설치 검증 후 Claude 바로 실행 (로그인 화면까지 연결)
# =====================================================================
...
function Invoke-ClaudeOneClickInstall {
    ...
    # --- [2/6] 이미 설치되어 있는지 확인 ---------------------------------
    Write-Step "2/8" "Claude Code가 이미 설치되어 있는지 확인하는 중..."
    ...
    Write-Step "3/8" "Git for Windows를 확인하는 중... (없어도 설치는 계속됩니다)"
    ...
    if ($alreadyInstalled) {
        Write-Step "4/8" "설치 단계 건너뜀 (이미 설치됨)"
    } else {
        Write-Step "4/8" "Claude Code를 설치하는 중... (30초~2분 정도 걸립니다)"
    ...
    Write-Step "5/8" "PowerShell에서 'claude' 명령을 바로 쓸 수 있게 설정하는 중..."
    ...
    Write-Step "6/8" "Node.js를 확인하는 중... (없으면 함께 설치합니다)"
    ...
    # --- [7/8] GitHub CLI + Vercel CLI (보너스 - 실패해도 계속 진행) -------
    Write-Step "7/8" "추가 개발 도구를 확인하는 중... (GitHub CLI, Vercel CLI)"

    # GitHub CLI (gh)
    if (Get-Command gh -ErrorAction SilentlyContinue) {
        Write-Ok "GitHub CLI - 이미 설치되어 있음"
    } elseif (Get-Command winget -ErrorAction SilentlyContinue) {
        Write-Warn2 "GitHub CLI를 설치합니다... (1~2분)"
        winget install --id GitHub.cli -e --source winget --silent --accept-package-agreements --accept-source-agreements 2>$null
        if ($LASTEXITCODE -eq 0) {
            # 방금 설치된 gh를 현재 세션에서 바로 쓸 수 있게 PATH 반영
            $ghDir = Join-Path $env:ProgramFiles 'GitHub CLI'
            if ((Test-Path $ghDir) -and -not (($env:Path -split ';') -contains $ghDir)) {
                $env:Path = "$env:Path;$ghDir"
            }
            Write-Ok "GitHub CLI 설치 완료"
        } else {
            Write-Warn2 "GitHub CLI 자동 설치에 실패했지만 계속 진행합니다. (https://cli.github.com)"
        }
    } else {
        Write-Warn2 "자동 설치 도구(winget)가 없어 GitHub CLI 설치를 건너뜁니다."
    }

    # Vercel CLI (npm 필요 - Node.js 단계가 성공했을 때만 가능)
    if (Get-Command vercel -ErrorAction SilentlyContinue) {
        Write-Ok "Vercel CLI - 이미 설치되어 있음"
    } elseif (Get-Command npm -ErrorAction SilentlyContinue) {
        Write-Warn2 "Vercel CLI를 설치합니다... (1~2분)"
        npm install -g vercel 2>$null | Out-Null
        # npm 전역 설치 경로를 현재 세션 PATH에 반영
        ...
        if (Get-Command vercel -ErrorAction SilentlyContinue) {
            ...
            Write-Warn2 "Vercel CLI 자동 설치에 실패했지만 계속 진행합니다. (https://vercel.com/docs/cli)"
        }
    } else {
        Write-Warn2 "Vercel CLI는 Node.js(npm)가 필요해서 건너뜁니다."
    }

    # --- [8/8] 설치 검증 ---------------------------------------------------
    Write-Step "8/8" "설치가 잘 됐는지 확인하는 중..."
